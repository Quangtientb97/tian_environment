...
sys.path.append("/coasiasemi/home/tientq/my_python_tools/csv_regdef/tian/")
import tian

home = ""
source_location = "/coasiasemi/home/tientq/my_python_tools/csv_regdef/"
...
def gen_ip_reg_def(link_xml, dir_output):
	...
	if dir_output == "":
		...
		file_out_link = "./csv_ip_regdef/{}_reg_blk.sv".format(name_blk_regdef.lower())
	else:
		if dir_output[-1] != '/':
			dir_output = dir_output + '/'
		if os.path.exists("{}/csv_ip_regdef".format(dir_output)) == 0:
			tian.run("mkdir {}/csv_ip_regdef".format(dir_output))
		file_out_link = dir_output + "csv_ip_regdef/{}_reg_blk.sv".format(name_blk_regdef.lower())

	file_out = []

	file_out.append("""\
//--------------------------------------------------------
// File Name   : {}_reg_def.sv
// Description : {} register definition
// Developers  : tian(tientq@coasia.com)
// Created	 :
// Generator   : csv_regdef
//--------------------------------------------------------
""".format(name_blk_regdef.lower(), name_blk_regdef.lower()))
	...
	for addressBlock in root.iter('addressBlock'):
		...
		register_data = tian.sub_in_list(register_data, "__NAME_REGISTER_DEFINE__"	   , "SUV_{}_{}_C".format(name_blk_regdef , r_name))
		register_data = tian.sub_in_list(register_data, "__REGISTER_NAME__"			  , "{}_{}".format(name_blk_regdef.lower() , r_name.lower()))
		register_data = tian.sub_in_list(register_data, "__REGISTER_NAME_WITH_BLK_NAME__", "{}".format(r_name.lower()))
		...
		if register.findall('field') != []:
			...
			r_field_name = "{}".format(r_name.lower())
			UVM_FIELD_PROPERTY.append("\trand uvm_reg_field {};\n".format(r_field_name))
	...
	for addressBlock in root.iter('addressBlock'):
		...
		register_data = tian.sub_in_list(register_data, "SUV___NAME_BLK___REG_BLK_C", "SUV_{}_REG_BLK_C".format(name_blk_regdef))
		register_data = tian.sub_in_list(register_data, "__NAME_BLK___reg_blk_c", "{}_reg_blk_c".format(name_blk_regdef.lower()))
		register_data = tian.sub_in_list(register_data, "__NAME_BLK___reg_blk_c", "{}_reg_blk_c".format(name_blk_regdef.lower()))
		...
		for register in addressBlock.findall('register'):
			...
			REG_BLK_C_PROPERTY.append("\trand {}_{}_c {};\n".format(name_blk_regdef.lower(), r_name, r_name))

			BUILD_REGISTER.append("\t\tthis.{} = {}_{}_c::type_id::create(\"{}\");\n".format(r_name, name_blk_regdef.lower(), r_name, r_name))
			BUILD_REGISTER.append("\t\tthis.{}.configure(this, None);\n".format(r_name))
			BUILD_REGISTER.append("\t\tthis.{}.build();\n\n".format(r_name))

			BUILD_REGISTER.append("\t\tdefault_map.add_reg({}, 'h{});\n\n\n\n".format(r_name, r_addressOffset.replace("0x", "")))
	...
	os.system("cat {} >> {}".format(file_out_link + "part2", file_out_link))
	os.system("rm -rf {}".format(file_out_link + "part2"))
	os.system("cp -rf {}/lib/reg_def_defines.sv {}csv_ip_regdef/".format(source_location, dir_output))
	os.system("sed -i 's/\r//g' {}".format(file_out_link))

def gen_soc_reg_def(link_config, dir_output, gen_soc_env_only = 0):
